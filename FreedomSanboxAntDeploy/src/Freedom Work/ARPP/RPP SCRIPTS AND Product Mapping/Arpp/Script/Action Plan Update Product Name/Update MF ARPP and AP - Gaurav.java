...
List<ARPP_Detail__c> lstArppDetail = [Select Name, Id, Entity__c, Type__c, Product__c, Commission__c, Approve_Action_Plan__c,  											  Action_Plan_Amount__c, Action_Plan_Upfront_Comm_Amount__c,  	
...
for(Action_Plan_Product__c objActionPlanProduct : [select Action_Plan_Product_Name__c, Product_Name__c 
												   ...
												   where Action_Plan_Product_Name__c IN: mapMFproductNameTolstAP.keySet() and      CreatedDate = TODAY ])
...
List<Product_Master__c> lstPM = [Select Product_Name__c, ProductType__c, Investment_Type__c, Policy_Type__c,
								...
								From Product_Master__c where  ProductType__c = 'Mutual Fund' Or ProductType__c = 'Life 
									 Insurance'    
...
for(String strProductName : mapMFproductNameTolstAP.keySet())
{
	...
	if(objPM != null)
	{
		...
		for(Approve_Action_Plan__c objAAP : mapMFproductNameTolstAP.get(strProductName))
		{
			...
			objARPP.Remark__c = '';
			lstARPPUpdatelstARPPUpdate.add(objARPP);
		}
	}
}
...
for(String strProductName : mapLIpolicyNameTolstAP.keySet())
{
	...
	if(objPM != null)
	{
		...
		for(Approve_Action_Plan__c objAAP : mapLIpolicyNameTolstAP.get(strProductName))
		{
			...
			lstAAPUpdate.add(objAAP);
			ARPP_Detail__c objARPP = mapApIdToARPP.get(objAAP.Id);
			if(objAAP.Item_type__c == 'Life Insurance' || objAAP.Item_type__c == 'General Insurance')
			{
				objARPP.Action_Plan_Upfront_Comm_Amount__c = objPM != null && objComm != null ?(objAAP.Premium_Amount_Rs__c != null ?(objComm.Upfront_Commission__c * objAAP.Premium_Amount_Rs__c)/100 : 0) : 0;
