...
//Update MF and Insurance type ARPP with ET 
List<ARPP_Detail__c> lstArppDetail = [Select Name, Id, Entity__c, Type__c, Product__c, Commission__c, Approve_Action_Plan__c, 
											Action_Plan_Amount__c, Action_Plan_Upfront_Comm_Amount__c, Action_Plan_Trial_Comm_Amount__c,   Action_Amount_Revenue__c, Remark__c, Execution_Tracker_Actual_Product__c,  Actual_Commission__c, Execution_Tracker__c, Execution_Tracker_Amount__c
									  ...
									  Where Execution_Tracker_Actual_Product__c = null and 
											...
											ET_Remark__c = 'Actual Product Commision not found' and  Entity__c ='001W000000E3ThzIAF'];
...
for(Execution_Tracker__c objET : [Select Id, Approve_Action_Plan__c 
								  from Execution_Tracker__c 
								  where ParentExecutionTracker__c = NULL and 
								  Approve_Action_Plan__c IN: mapApIdToARPP.keySet()])
{
...
Map<Id, Execution_Tracker__c> mapIdToET = new Map<Id, Execution_Tracker__c>([Select Id, Type__c, Approve_Action_Plan__r.Installments__c, 																						Approve_Action_Plan__r.Item_Type__c, 
																						Execution_Tracker_Product__c,  Executed_Product_Name__c ,Approve_Action_Plan__r.Tenure_of_Insurance__c 
																			From Execution_Tracker__c 
																			Where Id IN : mapETIdToARPP.keySet()]);
																			
system.debug('************mapIdToET********'+mapIdToET);	
...
system.debug('************mapLIExecutedProductNameTolstET********'+mapLIExecutedProductNameTolstET);	
...
for(Action_Plan_Product__c objActionPlanProduct : [select Action_Plan_Product_Name__c, Product_Name__c 
												   ...
												   where Action_Plan_Product_Name__c IN: mapLIExecutedProductNameTolstET.keySet()])
...
List<Product_Master__c> lstPM = [Select Product_Name__c, ProductType__c, Investment_Type__c, Policy_Type__c,
										...
										From Product_Master__c where  ProductType__c = 'Mutual Fund' or 
										ProductType__c ='General Insurance' or ProductType__c = 'Life Insurance'];
										
system.debug('************lstPM********'+lstPM);			
...
for(String strExecutedProudctName : mapLIExecutedProductNameTolstET.keySet())
{
	...
	if(objPM != null)
	{
		...
		for(Execution_Tracker__c objET : mapLIExecutedProductNameTolstET.get(strExecutedProudctName))
		{
			...
			{
				objARPP.Execution_Tracker_upfront_Comm_Amount__c = objComm != null ? (objARPP.Execution_Tracker_Amount__c * objComm.Upfront_Commission__c )/100.0 : 0;
				objARPP.Execution_Tracker_Trial_Comm_Amount__c = objComm != null ? (objARPP.Execution_Tracker_Amount__c * objComm.Trail_Commission__c )/100.0 : 0;
			}
			if(	objET.Approve_Action_Plan__r.Item_Type__c =='Life Insurance' || objET.Approve_Action_Plan__r.Item_Type__c =='General Insurance' )
			{
				objARPP.Execution_Tracker_upfront_Comm_Amount__c = objComm != null  ? (objComm.Upfront_Commission__c * objARPP.Execution_Tracker_Amount__c)/100 : 0;
